...
test_north = test_results.sel(edge_type=0).dropna(dim="longitude")

spl, u = scipy.interpolate.make_splprep([test_north.longitude, test_north], s=5)

ax = plot_cwv_field(test, levels=levels_cwv, extent=extent)
add_east_west_boxes(ax)
ax.plot(spl(u)[0] - 360, spl(u)[1])
# %%

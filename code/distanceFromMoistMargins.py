...
test_longitude=360-40
test_cwv = cwv_orcestra_mean.sel(longitude=test_longitude)

test_lat_min, test_lat_max = find_edges(test_cwv, cwv_max_lat_smooth.sel(longitude = test_longitude), 45)

plt.scatter([test_longitude, test_longitude], [test_lat_min, test_lat_max])
# %%

# %%

def find_edges_numpy(cwv, latitudes, lat_cwv_max, cwv_thresh, cwv_min=0):
    """
    Pure NumPy version of find_edges.

    Parameters:
        cwv (np.ndarray): 1D array of CWV values.
        latitudes (np.ndarray): 1D array of corresponding latitudes (same shape as cwv).
        lat_cwv_max (float): Latitude of CWV peak in the mean profile.
        cwv_thresh (float): CWV threshold to define the edge.
        cwv_min (float): Minimum CWV to define moist region.

    Returns:
        [lat_south, lat_north] (both float): Southernmost and northernmost latitude where CWV equals threshold,
                                             within the moist region centered near lat_cwv_max.
    """
    ...
    if np.any(north_cwv <= cwv_thresh):
        lat_north = np.min(north_lats[north_cwv <= cwv_thresh])
    else:
        lat_north = np.nan

    # Southern side: latitudes <= peak
    ...
    if np.any(south_cwv >= cwv_thresh):
        lat_south = np.min(south_lats[south_cwv >= cwv_thresh])
    else:
        lat_south = np.nan

    return np.array([lat_south, lat_north], dtype=float)

#%%
results = xr.apply_ufunc(
    find_edges_numpy,
    cwv_orcestra_mean,
    cwv_orcestra_mean.latitude,
    cwv_max_lat_smooth, 
    45,
